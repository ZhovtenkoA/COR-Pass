...
# #                     #    current_user: UserDb = Depends(Auth.get_current_active_user),
...
# #     """
# #     print(current_user)
# #     record = await repository_record.create_record(body, current_user, db, encryption_key)
# #     return record


# # @router.put("/{record_id}", response_model=ResponseRecord, dependencies=[Depends(user_access)])
# # async def update_record(record_id: int, body: CreateRecordModel, db: Session = Depends(get_db), 
# #                         # current_user: UserDb = Depends(Auth.get_current_active_user)
# #                         ):
# #     """
